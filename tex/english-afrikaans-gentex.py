        ...
        for book,chapter,verse,text in self.booktochapters():
            ...
            if ( chapter!=ochapter or book!=obook ):
                if ochapter:
                    ...
                    yield { 'book':obook, 'chapter': ochapter, 'paragraph': self.state['paragraph'], 'chaptertext': ''.join(chaptertext), }
                    self.state['paragraph']=0
                    chaptertext=[]
                if book!=obook:
    ...
    for left,right in zip( en.parsebooks(), af.parsebooks() ):
        ...
        if 'newbook' in left and 'newbook' in right:
            ...
            o+=   r'\begin{paracol}{2}\biblleftcolumn{english}' \
                + left['chaptertext'].strip()+'%\n' \
                + r'\biblrightcolumn{afrikaans}%' + '\n' \
                + right['chaptertext'].strip()+'%\n' \
                + r'\bibldonecolumns' + '\n' \
