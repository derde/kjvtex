...
\newfontfamily\myfootnotefont[Scale=0.80]{\myfontname} % Used for (synthetic) smallcaps
...
\newcommand{\biblendchapter}[2]{\markright{#1}\def\mybookindex{#2}}%
\newcommand{\biblnewbook}[3]{\markboth{#1}{#1}\def\mybookshort{#2}\def\mybookindex{#3}}%
%
...
\addfontfeature{LetterSpace=-3 plus 3 minus 0}% The 3m space is what a good font design uses ... and we trample it
\mainmatter%
...
\vfill
\par
Typesetting by \LaTeX{} in \makeatletter \f@family \makeatother\ \the\fontdimen6\font\the\font\ text, / line spacing \the\baselineskip
\par
