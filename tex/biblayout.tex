...
\TPGrid[0mm,7mm]{26}{66}% defines tiny boxes down right hand margin for book name - see textblock below
...
\fancyhead[OR]{\myheadingfont \textbf{\textemdash\ \leftmark}\biblthumbmark{\mythumbindex}\oddbindingmarks}% Top right header
...
\newfontfamily\myendnotesfont[\myboldness FakeStretch=\myfontstretch,Scale=\mychapterscale]{\mysansfont}
...
\newcommand {\versei}[2]{\mypreversei \hebrewleader\def\hebrewleader{}}%3 points up, count height as 0 points, kern at end
...
\newcommand{\biblthumbmark}[1]{\begin{textblock}{1}(25,#1)\hfill\rule[0em]{1em}{0.5em}\end{textblock}}
%\renewcommand{\bibltwocolumn}{\par}%
%\renewcommand{\biblonecolumn}{\par}%
\newcommand{\biblendbook}[1]{\needspace{3\baselineskip}\biblonecolumn}%
...
\def\bookheadingstrut{%
%\vskip\baselineskip% disappearing space when at the top of the page? don't work :(
\rule[-1\baselineskip]{0pt}{3\baselineskip}% Number of lines for title
}%
...
\newcommand{\biblbookheading}[1]{%
    \needspace{6\baselineskip}\null% This causes the leading space at the top of a space not to be "consumed" - Genesis and Matthew ... still not exactly right though
    {%
    \begin{center}
    \bookheadingstrut% A multi-line strut for the big text
    \raisebox{-0.5\baselineskip}[0pt]{\textbf{\mybooktitlefont\mybooktitlesettings #1}}
    \end{center}%
    }
    \bibltwocolumn
}%
...
\lettrine[lines=2, lraise=0.05, lhang=0.00, findent=3pt, nindent=0pt, loversize=0.0]{\hspace{2pt}\textbf{\mychapterfont #1}}{}}%
...
HOLY BIBLE\par
King James Bible: \textsc{kjvtex compact}\par
...
\par
Source text TEXT-PCE-127.txt, d6176dd776234eeaf0944dd0cac24bef MD5
\par
\newcounter{mylines}
...
\null
%\bookheadingstrut\par
\vfill
\begin{center}%
\textsc{\ENpretitle}%HOLY BIBLE
\par\null
\textbf{\mybooktitlefont{\ENtitle}}% King James Bible
\par
\textsc{\ENsubtitle}% kjvtex compactt
\par
\vfill
\par
\ENmotto
...
\newcommand{\biblnewbook}[2]{\write\mytoc{\noindent #1 \unexpanded{\mydotfill}{\thepage}\par}\gdef\mybookshort{#2}\gdef\mythumbindex{#2}}%
